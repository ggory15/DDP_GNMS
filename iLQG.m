function [x, u, L, Vx, Vxx, cost, trace, stop] = iLQG(DYNCST, x0, u0, M, Op)
...
for i=0:M-1
   xnew(:,:,l*i+1:l*(i+1)) = x_split(:,:, 1:l, i+1);
   unew(:,:,l*i+1:l*(i+1)) = u_split(:,:, 1:l, i+1);
...
function [diverge, Vx, Vxx, k, K, dV] = back_pass(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,regType,lims,u, dn)
...
for i = N-1:-1:1
    ...
    Qu  = cu(:,i)      + fu(:,:,i)'* (Vx(:,i+1) + Vxx(:,:,i+1) * dn(:,i));
    Qx  = cx(:,i)      + fx(:,:,i)'*Vx(:,i+1);
